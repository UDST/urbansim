...
for dirname in os.listdir('base_year_data/2010/'):
    if dirname not in ['parcels','buildings','households','jobs','zones','travel_data','annual_employment_control_totals','annual_household_control_totals','annual_household_relocation_rates','annual_job_relocation_rates','building_sqft_per_job','building_types','counties','target_vacancies','development_event_history']: continue
    table_path = 'base_year_data/2010/' + dirname
    print dirname
    df = cache_to_df(table_path)
    keys = [dirname[:-1]+"_id"]
    if dirname == "travel_data": keys = ["from_zone_id","to_zone_id"]
    if dirname == "annual_employment_control_totals": keys = ["sector_id","year","home_based_status"]
    if dirname == "annual_job_relocation_rates": keys = ["sector_id"]
    if dirname == "annual_household_control_totals": keys = ["year"]
    if dirname == "annual_household_relocation_rates": keys = ["age_of_head_max","age_of_head_min","income_min","income_max"]
    if dirname == "building_sqft_per_job": keys = ["zone_id", "building_type_id"]
    if dirname == "counties": keys = ["county_id"]
    if dirname == "development_event_history": keys = ["building_id"]
    if dirname == "target_vacancies": keys = ["building_type_id","year"]
    if dirname <> "annual_household_relocation_rates": df = df.set_index(keys)
