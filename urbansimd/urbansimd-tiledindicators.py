...
def compute_variable(table, variable, xmin, ymin, xmax, ymax, srid=None,
                     minmaxonly=0, sampleprop=None, qcut=0, bins=0):
    ...
    global GEOMXYS
    if not minmaxonly:
        xmin, ymin, xmax, ymax = float(xmin), float(
    ...
    if bins > 0 and not qcut:
        bins = [(mx - mn) / bins * (i + 1) + mn for i in range(bins - 1)]
    if bins > 0 and qcut:
        bins = [stacked.quantile((i + 1.0) / bins) for i in range(bins - 1)]
    ...
    if not minmaxonly:
        if vector.values.ndim == 2:
            to_json['values'] = dict(
                zip([int(x) for x in vector.index],
                    [list(x) for x in vector.values]))
        else:
            to_json['values'] = dict(
                zip([int(x) for x in vector.index],
                    [[float(x)] for x in vector.values]))
...
def serve_ajax():
    ...
    return compute_variable(
        tables[0], variable, bbox[0], bbox[1], bbox[2], bbox[3], srid,
        minmaxonly, sampleprop, qcut, bins)


def start_service(geomxys, tabledict, port, host='localhost', srid=3740,
                  debug=1, thread=0):
    try:
        settings.configure()
    except:
    ...
    if not INIT:
        ...
        if not thread:
            ...
            t = threading.Thread(
                target=run,
                kwargs={'host': host, 'port': port, 'server': 'tornado'})
            t.daemon = True
            t.start()
...
if __name__ == '__main__':
    ...
    print time.ctime()
    geomxys = pd.DataFrame(
        {'x': centroid.apply(lambda x: x.x),
         'y': centroid.apply(lambda y: y.y)},
