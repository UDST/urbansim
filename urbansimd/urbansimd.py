...
def query():
  ...
  if 'key_dictionary' in req:
    ...
    dictionary = json.loads(dictionary_file)                                    #the dictionary is in unicode, is it necessary
    print dictionary
    recs = [[dictionary[str(int(x))],float(recs.ix[x])] for x in recs.index]    #the dictionary has keys from 0 to 15, ids come from 0 to 16
  else:
