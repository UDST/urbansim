...
def sample_rows(total, data, replace=True, accounting_column=None, max_iterations=50):
    """
    Samples and returns rows from a data frame while matching a desired control total. The total may represent a simple 
    row count or may attempt to match a sum/quantity from an accounting column. 
    ...
    if accounting_column is None:
        if replace == False and total > len(data.index.values):
    ...
    if replace == False and total > data[accounting_column].sum():
        raise ValueError('Control total exceeds the available samples')

    # determine avg number of accounting items per sample (e.g. persons per household, jobs per business)
