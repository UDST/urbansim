...
# the keyword "inplace" for all of these functions re-uses the memory wherever possible
# use this if you don't want allocate more memory on the gpu which can be expensive,
# and don't need to reuse the matrix being transformed
...
class PMAT:
    ...
    # find the first positive value along an axis - for use in doing random choice
    ...
    def reshape(self,rowlen,collen):
      if rowlen == -1: rowlen = self.size()/collen
      if collen == -1: collen = self.size()/rowlen
      if self.typ == 'numpy':
