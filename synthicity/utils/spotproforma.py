...
class Developer:
 ...
 def generate_lookup(self):
  ...
  for name in keys:
    ...
    for parking_config in parking_configs:
      ...
      break_even_weighted_rent = PROFITFACTOR*yearly_cost_per_sqft*-1.0
 ...
 def lookup(self,form,rents,land_costs,parcel_sizes,max_fars,max_heights):
  ...
  maxprofit_fars = pd.Series(even_rents.index.values[maxprofitind].astype('float'),index=parcel_sizes.index) 
